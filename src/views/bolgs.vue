...
import {BlogsInt, InitData} from "../type/blog";
...
export default {
  ...
  setup() {
    ...
    const onSubmit=()=>{
      let arr:BlogsInt[] = []
      if (data.query.title){
        arr=data.list.filter((value)=>{return value.title.indexOf(data.query.title)!==-1})
      }else {
        arr=data.list
      }
      data.list=arr
    ...
    return {...toRefs(data), currentChange, sizeChange,onSubmit}
  }
}
</script>
